...
<body>
  <header class="site-header">
    ...
    <nav><a href="/">Countries</a><a href="/projects/">Projects</a><a href="/data/latest/manifest.json">JSON</a></nav>
  </header>
  ${body}
...
function countryPage(snapshot: RankingSnapshot<RankedContributor>): string {
  const description = `Top GitHub contributors in ${snapshot.name}, sorted by public commit activity with public PR, repo, and follower columns.`;
  return layout(`Top GitHub contributors in ${snapshot.name}`, description, `<main class="page"><section class="hero compact"><p class="eyebrow">Country rankings</p><h1>Top GitHub contributors in ${escapeHtml(snapshot.name)}</h1><p>Sorted by one-year commit totals by default. Click the column buttons to sort by public PRs, repos, or followers instead.</p><div class="stats"><span>${snapshot.entries.length} contributors</span><span>${snapshot.candidate_count} candidates checked</span><span>Updated ${escapeHtml(snapshot.generated_at.slice(0, 10))}</span></div></section><section class="panel">${contributorTable(snapshot)}<p class="note">Commit and PR counts are one-year public contribution totals from the official GitHub GraphQL API. Repo and follower counts come from public GitHub profile metadata. Country matching uses free-text GitHub profile location and is not verified geography.</p><p class="note"><a href="/data/latest/countries-${snapshot.slug}.json">Raw JSON snapshot</a></p></section></main>`);
}
...
await writePage('', layout('Top GitHub contributors by country', 'Choose a country to see top GitHub contributors ranked by commits, public PRs, repos, and followers.', `<main><section class="hero"><p class="eyebrow">GitHub country rankings</p><h1>Top GitHub contributors by country.</h1><p>Pick a country, then compare contributors by commits, public pull requests, repositories, and followers. No blended score needed for the main flow.</p>${countrySelector(countries)}</section><section class="page"><section class="panel"><h2>Projects</h2><p>There is also one simple projects page for public project activity.</p><a href="/projects/">View projects</a></section><section class="panel"><h2>Data status</h2><p>Generated ${escapeHtml(manifest.generated_at)} from ${escapeHtml(manifest.api_budget.mode)} data.</p><a href="/data/latest/manifest.json">Open manifest JSON</a></section></section></main>`));
for (const country of countrySnapshots) {
  await writePage(`countries/${country.slug}`, countryPage(country));
  await writePage(`countries/${country.slug}/top-github-contributors`, countryPage(country));
}
