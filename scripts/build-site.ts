...
const assetVersion = '20260509-fix4';
...
function countrySelector(countries: Array<{ name: string; slug: string; entries: number; status: string }>): string {
  return `<div class="country-grid">${countries.map((country) => `<a class="country-card" href="/countries/${country.slug}/top-github-contributors/"><strong>${escapeHtml(country.name)}</strong><span>${country.entries.toLocaleString()} ranked contributors</span><span>Status: ${escapeHtml(country.status)}</span><span>Open leaderboard →</span></a>`).join('')}</div>`;
}

function boardCards(cards: Array<{ title: string; text: string; href: string; icon?: string }>): string {
  return `<div class="board-grid">${cards.map((card) => `<a class="board-card" href="${card.href}"><span class="board-icon" aria-hidden="true">${escapeHtml(card.icon ?? '↗')}</span><strong>${escapeHtml(card.title)}</strong><span>${escapeHtml(card.text)}</span></a>`).join('')}</div>`;
}
...
await writePage('', layout('Top GitHub contributors by country', 'Choose a country to see top GitHub contributors ranked by commits, public PRs, repos, and followers.', `<main><section class="hero"><p class="eyebrow">Public OSS radar</p><div class="hero-shell"><div class="hero-copy"><h1>Find real OSS signal.</h1><p>Weekly GitHub snapshots for discovering active contributors, rising builders, and fast-moving projects.</p><div class="hero-actions"><a class="button-link" href="#countries">Browse country boards</a><a class="button-link secondary" href="/global/">Open global leaderboard</a></div><div class="stats"><span>${countries.length} country boards</span><span>${categorySnapshots.length} project categories</span><span>${updateCadenceLabel} · Latest ${escapeHtml(manifest.generated_at.slice(0, 10))}</span></div></div><aside class="hero-card"><h2>Scan fast</h2><ul><li><span>Contributors</span> <strong>Country and global leaderboards</strong></li><li><span>Projects</span> <strong>Momentum, agentic, and category views</strong></li><li><span>Method</span> <strong>GitHub-only, auditable discovery queries</strong></li></ul></aside></div>${boardCards([{ title: 'Countries', text: `${countries.length} observed country boards`, href: '#countries', icon: '🌏' }, { title: 'Agentic projects', text: 'Agent, Claude, Codex, and OpenClaw views', href: '/agentic/', icon: '🤖' }, { title: 'Momentum map', text: 'Velocity versus legitimacy for projects', href: '/momentum/', icon: '📈' }, { title: 'Rising contributors', text: 'High activity relative to audience size', href: '/rising/', icon: '✨' }])}</section><section id="countries" class="page"><div class="section-heading"><div><h2>Country boards</h2><p>Pick a geography to see who is shipping consistently, then jump into raw snapshots or methodology when you need to verify the signal.</p></div></div>${countrySelector(countries)}<section class="panel"><h2>Data status</h2><p>Generated ${escapeHtml(manifest.generated_at)} from ${escapeHtml(manifest.api_budget.mode)} data. OSSRank refreshes on a weekly cadence from public GitHub API snapshots, with occasional manual refreshes when needed.</p><p class="note"><a href="/data/latest/manifest.json">Open manifest JSON</a></p></section></section></main>`, ''), { priority: '1.0' });
