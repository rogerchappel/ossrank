...
<head>
  ...
  <script>
    const savedTheme = localStorage.getItem('ossrank-theme');
...
<body>
  ...
  <script>
    ...
    function setTheme(theme) {
      ...
      localStorage.setItem('ossrank-theme', theme);
      if (themeToggle) themeToggle.textContent = theme === 'dark' ? '☀️' : '🌙';
      document.querySelector('meta[name="theme-color"]')?.setAttribute('content', theme === 'dark' ? '#07111f' : '#fff8ed');
    }
...
function rankMovementBadge(entry: RankedContributor): string {
  if (!entry.previous_rank) return `<span class="rank-delta rank-delta-new" title="New in the latest weekly OSSRank snapshot">new</span>`;
