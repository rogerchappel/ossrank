...
import { contributorCommitSignal, legitimacyScore, momentumScore } from '../src/lib/ranking.js';
...
const assetVersion = '20260828-icons1';
...
function statusTone(status: string): string {
  return status === 'fresh' || status === 'current' ? 'good' : status === 'failed' || status === 'stale' ? 'warning' : 'neutral';
}

function countrySelector(countries: Array<{ name: string; slug: string; entries: number; status: string }>): string {
  return `<div class="country-grid">${countries.map((country) => `<a class="country-card" href="/countries/${country.slug}/top-github-contributors/"><span class="card-kicker">${statusPill(statusLabel(country.status), statusTone(country.status))}</span><strong>${escapeHtml(country.name)}</strong><span>${country.entries.toLocaleString()} ranked contributors</span><span class="card-link">Open leaderboard →</span></a>`).join('')}</div>`;
}
...
function commitMetric(entry: RankedContributor): string {
  const raw = entry.observed_public_commits ?? entry.public_contributions;
  const adjusted = contributorCommitSignal(entry);
  if (!entry.contribution_burst_adjustment) return `<strong class="metric">${raw.toLocaleString()}</strong>`;
  const adjustment = entry.contribution_burst_adjustment;
  return `<strong class="metric">${adjusted.toLocaleString()}</strong><small title="Raw GitHub total: ${raw.toLocaleString()}. Adjusted because ${adjustment.capped_days} day${adjustment.capped_days === 1 ? '' : 's'} exceeded the per-user burst cap.">raw ${raw.toLocaleString()} · burst-adjusted</small>`;
}
...
function contributorTable(snapshot: RankingSnapshot<RankedContributor>): string {
  ...
  return `<div class="table-wrap"><table id="contributors" aria-label="Top GitHub contributors in ${escapeHtml(snapshot.name)}"><thead><tr><th>Rank</th><th>Movement</th><th>Username</th><th><button data-sort="commits">Ranked commits (1y)</button></th><th><button data-sort="prs">Public PRs (1y)</button></th><th><button data-sort="repos">Repos</button></th><th><button data-sort="followers">Followers</button></th><th>Location</th><th>Discovery</th></tr></thead><tbody>${rows.map((entry) => `<tr data-commits="${contributorCommitSignal(entry)}" data-prs="${entry.observed_public_pull_requests ?? 0}" data-repos="${entry.public_repos ?? 0}" data-followers="${entry.followers}"><td class="rank"><span class="rank-pill">#${entry.rank}</span></td><td>${rankMovementBadge(entry)}</td><td><div class="contributor-name"><a href="${entry.profile_url}" ${externalLinkAttrs}>${escapeHtml(entry.login)}</a></div><small>${escapeHtml(entry.name ?? 'Public GitHub profile')}</small></td><td>${commitMetric(entry)}</td><td><strong class="metric">${(entry.observed_public_pull_requests ?? 0).toLocaleString()}</strong></td><td>${(entry.public_repos ?? 0).toLocaleString()}</td><td>${entry.followers.toLocaleString()}</td><td>${escapeHtml(entry.location ?? 'Unknown')}<small>${escapeHtml(entry.location_confidence ?? 'unknown')}</small></td><td>${statusPill(entry.discovered_by_query ?? 'query', 'source')}</td></tr>`).join('')}</tbody></table></div><script>
...
let countrySnapshots = await Promise.all(countryShards.map((shard) => readJson<RankingSnapshot<RankedContributor>>(shardPath(shard))));
if (countrySnapshots.length === 0) {
  const countryFiles = (await readdir(dataDir)).filter((file) => /^countries-.+\.json$/.test(file)).sort();
  countrySnapshots = await Promise.all(countryFiles.map((file) => readJson<RankingSnapshot<RankedContributor>>(join(dataDir, file))));
}
...
const dataStatusBadge = statusPill(statusLabel(manifest.status), statusTone(manifest.status));
