...
function contributorTable(snapshot: RankingSnapshot<RankedContributor>): string {
  return `<table><thead><tr><th>Rank</th><th>Contributor</th><th>Observed public contributions</th><th>Followers</th><th>Location signal</th><th>Move</th></tr></thead><tbody>${snapshot.entries.map((entry) => `<tr><td>#${entry.rank}</td><td><a href="${entry.profile_url}">${escapeHtml(entry.login)}</a><small>${escapeHtml(entry.name ?? '')}</small></td><td>${entry.public_contributions.toLocaleString()}</td><td>${entry.followers.toLocaleString()}</td><td>${escapeHtml(entry.location ?? 'Unknown')}<small>${escapeHtml(entry.location_confidence ?? 'unknown')}</small></td><td>${movement(entry)}</td></tr>`).join('')}</tbody></table>`;
}
